  ...
  y = function (i, o) {
    return new Promise(function (n, r) {
      ...
      (e.onload = function () {
        ...
        (e.onerror = function () {
          t(), r(new Error('Failed to load the image "' + i + '"'));
        }),
        (e.src = i);
    });
  };
const F = "/RetroFilmDate/example.png",
  D = "/RetroFilmDate/fonts/alarmClock.ttf",
  P = "/RetroFilmDate/fonts/sixtyfour.ttf",
  q = "/RetroFilmDate/fonts/BebasNeue.ttf";
async function I(i) {
  ...
  for (let n = 0; n < o.length; n++) {
    ...
    (e.onload = async function (t) {
      const a = t.target.result,
        l = r.name,
        d = document.querySelector("#fontSelector"),
        m = document.querySelector("#dateFormatSelector"),
        c = document.querySelector("#customDateInput"),
        s = d.options[d.selectedIndex].value,
        g = m.value,
        w = c.value,
        v = await u(a, s, w, g),
        p = document.querySelector("#previewContainer"),
        S = document.createElement("img");
      S.src = v;
      const f = document.createElement("a");
      (f.href = v),
        (f.download = `stamped_${l}`),
        (f.textContent = `Download ${l}`),
        p.appendChild(f),
        p.appendChild(S),
...
async function u(i, o, n, r) {
  const e = await y(i),
    t = h(e.width, e.height),
    a = t.getContext("2d"),
    d = o || "./fonts/alarmClock.ttf",
    m = new FontFace("SelectedFont", `url(${d})`);
  await m.load(),
    document.fonts.add(m),
    a.drawImage(e, 0, 0, e.width, e.height),
    (a.font = "70px SelectedFont");
  let c = new Date();
  n ? (c = new Date(n)) : (c = new Date());
  let s = c.toLocaleDateString();
  return (
    r === "US"
      ? (s = c.toLocaleDateString("en-US", {
          day: "2-digit",
          month: "2-digit",
          year: "2-digit",
        }))
      : r === "UK" &&
        (s = c.toLocaleDateString("en-GB", {
          day: "2-digit",
          month: "2-digit",
          year: "2-digit",
        })),
    ...
    (a.textAlign = "right"),
    (a.shadowColor = "#ff1e0a"),
    (a.shadowBlur = 10),
    a.fillText(s, t.width - 50, t.height - 50),
...
function C() {
  ...
  (document.querySelector("#app").innerHTML = i),
    document.querySelector("#imageInput").addEventListener("change", I);
  const o = new Image();
  (o.onload = async function () {
    const n = await u(o.src);
    imagePreview.src = n;
  }),
    ...
    document.querySelector("#fontSelector").addEventListener("change", () => {
      ...
      let r = n.options[n.selectedIndex].value,
    ...
    document
      ...
      .addEventListener("change", () => {
        const n = fontSelector.value,
          r = dateFormatSelector.value;
        let e = document.querySelector("#imagePreview");
        (e = o.src),
          u(e, n, r).then((t) => {
            document.querySelector("#imagePreview").src = t;
          }),
    ...
    document
      .querySelector("#customDateInput")
      .addEventListener("change", () => {
        const n = document.querySelector("#fontSelector"),
          r = document.querySelector("#dateFormatSelector"),
          e = n.options[n.selectedIndex].value,
          t = r.value,
          a = document.querySelector("#customDateInput").value;
        let l = document.querySelector("#imagePreview").src;
        (l = o.src),
          u(l, e, a, t).then((d) => {
            document.querySelector("#imagePreview").src = d;
          });
      });
}
C();
