...
async function f(a) {
  ...
  (o.onload = async function (r) {
    ...
    d.href = c;
    const s = `stamped_${t}`;
    (d.download = s), d.removeAttribute("disabled");
  }),
    o.readAsDataURL(n);
}
async function g(a, n) {
  const o = await u(a),
    r = m(o.width, o.height),
    e = r.getContext("2d"),
  ...
  (t.textContent = `
    ...
    (e.font = "70px AlarmClock");
  const c = new Date(n).toLocaleDateString("en-US", {
    day: "2-digit",
    month: "2-digit",
    year: "2-digit",
  });
...
function p() {
  ...
  (document.querySelector("#app").innerHTML = a),
    document.querySelector("#imageInput").addEventListener("change", f);
}
p();
